...
Add native C++ visualisation and the mathplot smoke target:
...
The `qt-kit-*` presets live in `CMakeUserPresets.json` because they contain
machine-specific Qt paths. Keep that file local and do not commit it.
